...
func encodePacket(priv *ecdsa.PrivateKey, ptype byte, req interface{}) ([]byte, error) {
	...
	if err := rlp.Encode(b, req); err != nil {
		common.P2PLogger.Error("error encoding packet:", "reason", err)
		return nil, err
	}
	packet := b
	sig, err := crypto.Sign(crypto.Keccak256(packet.Bytes()[headSize:]), priv)
	if err != nil {
		common.P2PLogger.Error("could not sign packet:", "reason", err)
		return nil, err
	}
	copy(packet.Bytes()[macSize:], sig)
	// add the hash to the front. Note: this doesn't protect the
	// packet in any way. Our public key will be part of this hash in
	// The future.
	copy(packet.Bytes(), crypto.Keccak256(packet.Bytes()[macSize:]))
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	if err != nil {
		common.P2PLogger.Debug(fmt.Sprintf("Bad packet from %v: %v\n", from, err))
		return err
	}
	status := "ok"
	...
	common.P2PLogger.Debug(fmt.Sprintf("<<< %v %T: %s\n", from, packet, status))
	return err
}
...
func (req *findnode) handle(t *udp, from *net.UDPAddr, fromID NodeID, mac []byte) error {
	...
	target := types.BytesToHashPanic(crypto.Keccak256(req.Target[:]))
	t.mutex.Lock()
