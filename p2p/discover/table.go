...
// bootstrap nodes seed lookups while the table is empty.
type Table struct {
...
func (tab *Table) Bootstrap(nodes []*Node) {
	...
	for _, n := range nodes {
		cpy := *n
		cpy.sha = types.BytesToHashPanic(crypto.Keccak256(n.ID[:]))
			...
			if !asked[n.ID] {
				...
				go func() {
					...
					if err != nil {
						...
						common.P2PLogger.Debug(fmt.Sprintf("Bumping failures for %x: %d", n.ID[:8], fails))

						if fails >= maxFindnodeFailures {
							common.P2PLogger.Debug(fmt.Sprintf("Evacuating node %x: %d findnode failures", n.ID[:8], fails))
							tab.del(n)
						}
					}
					reply <- tab.bondall(r)
...
func (tab *Table) bond(pinged bool, id NodeID, addr *net.UDPAddr, tcpPort uint16) (*Node, error) {
	...
	var result error
	if node == nil || fails > 0 {
		common.P2PLogger.Debug(fmt.Sprintf("Bonding %x: known=%v, fails=%v", id[:8], node != nil, fails))
