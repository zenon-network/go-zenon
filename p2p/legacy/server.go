...
// Package legacy implements the original devp2p/RLPX networking stack.
// It is the pre-libp2p backend, kept alongside the new libp2p stack so the
// network can transition via a spork-gated activation rather than a hard cut.
package legacy
...
import (
	...
	nat "github.com/zenon-network/go-zenon/p2p/legacy/nat"
)
...
type transport interface {
	...
	p2p.MsgReadWriter
	// transports must provide Close because we use p2p.MsgPipe in some of
...
func (srv *Server) setupConn(fd net.Conn, flags connFlag, dialDest *discover.Node) {
	...
	if dialDest != nil && c.id != dialDest.ID {
		c.close(p2p.DiscUnexpectedIdentity)
