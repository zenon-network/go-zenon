...
func forEachUnpack(t Type, output []byte, start, size int) (interface{}, error) {
	...
	if start+WordSize*size > len(output) {
...
func toGoType(index int, t Type, output []byte) (interface{}, error) {
	...
	if t.requiresLengthPrefix() {
		...
		returnOutput = output[index : index+WordSize]
	}
	...
	case AddressTy:
		addr, _ := types.BytesToAddress(returnOutput[WordSize-types.AddressSize : WordSize])
		return addr, nil
	...
	case HashTy:
		return types.BytesToHash(returnOutput[WordSize-types.HashSize : WordSize])
	default:
...
func lengthPrefixPointsTo(index int, output []byte) (start int, length int, err error) {
	...
	bigOffsetEnd := intpool.Get().SetBytes(output[index : index+WordSize])
	...
	lengthBig := intpool.Get().SetBytes(output[offsetEnd-WordSize : offsetEnd])
