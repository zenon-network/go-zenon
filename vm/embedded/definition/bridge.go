...
const (
	...
	[
		...
		{"type":"variable","name":"securityInfo","inputs":[
			{"name":"requestedAdministratorPubKey","type":"string"},
	...
	ChangeAdministratorEDDSAPubKeyMethodName = "ChangeAdministratorEDDSAPubKey"
...
type BridgeInfoVariable struct {
	// Administrator pub key
	AdministratorEDDSAPubKey string `json:"administratorEDDSAPubKey"`
...
type SecurityInfoVariable struct {
	...
	RequestedAdministratorPubKey string `json:"requestedAdministratorPubKey"`
...
func (s *SecurityInfoVariable) Save(context db.DB) error {
	data, err := ABIBridge.PackVariable(
		...
		s.RequestedAdministratorPubKey,
...
func getUnwrapTokenRequestKey(transactionHash types.Hash, logIndex uint32) []byte {
	...
	return common.JoinBytes(unwrapTokenRequestKeyPrefix, transactionHash[:], logNumberBytes)
}
...
type ChangeEDDSAPubKeyParam struct {
	PubKey    string
	Signature string
}
